...
const Visual = z.enum(['person', 'chat-bubbles', 'agent', 'document', 'database', 'search', 'merge', 'schema', 'graph', 'link', 'clock', 'alert', 'idea', 'gear', 'waveform', 'microphone', 'headphones', 'music', 'globe', 'shield', 'layers', 'mask']);
...
async function main() {
  ...
  const storyboard = await parsedResponse(client, {
    ...
    instructions: [
      'You compose square, pastel hand-drawn technical infographic pages.',
      ...
      'Choose a semantic visual for every node. Use person only for an actual human actor, user, reader, customer, or operator mentioned in the source. Never add a mascot or permanent guide.',
