...
For normal blog publishing, select the best four or five complementary visual
arguments and generate 4-5 Pipeline 2 candidate GIFs. An uncurated first draft
may use all of them; human selection later replaces that set and triggers
pruning. More pages are for renderer canaries or an explicit user request, not
section-by-section illustration.

## Workflow

1. Read the complete source and extract its thesis, actors, evidence, constraints, transformations, and outcomes.
2. Rank possible visual arguments and keep the best 4-5 without using filler or duplicate visual jobs.
...
- A normal article gets 4-5 Pipeline 2 pages and candidate GIFs. Use fewer only when the source cannot support four strong visual arguments without filler; use more than five only for a canary, renderer development, or an explicit request.
...
- Use distinct, complementary layouts across the candidate set. Do not repeat topology or visual rhythm merely to reach four or five pages.
...
- Pick one page rhythm: route pulses, lane scanning, a stationary breathing funnel gate, timeline ticks, matrix scanning, orbit motion, schema rails, retrieval feedback, scatter/gather, hourglass filtering, or spiral refinement. Never sweep a decorative line up and down through funnel content.
...
- Preserve `section` and `outputFile` so approved GIF filenames remain stable unless a rename is intentional. `outputFile` must be a safe basename ending in `.gif`; the renderer adds and persists a numbered default when the field is absent.
...
- Also run the full storyboard after human curation removes pages. Do not use `--page` for pruning because only a full render removes stale binaries and rebuilds the complete manifest.
...
- Treat human acceptance and rejection as curation feedback. Preserve accepted outputs, regenerate only affected candidates, and turn repeated concerns about spacing, motion, icons, or topology into reusable pipeline improvements.
