...
const [, , diagramPath = 'diagram.json', htmlPath = 'output/diagram.html', framesDir = 'output/frames', outputPath = 'output/diagram.gif'] = process.argv;
...
(async () => {
  ...
  try {
    ...
    try {
      execFileSync('ffmpeg', ['-y', '-framerate', String(diagram.fps), '-i', `${absoluteFrames}/frame-%05d.png`, '-vf', 'palettegen=max_colors=128', '-frames:v', '1', '-update', '1', palette], { stdio: 'inherit' });
