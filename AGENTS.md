...
- Published Pipeline 1 cards and Pipeline 2 nodes must define an explicit canonical `icon` or `visual`. `npm run gif:check` enforces this contract.
...
- Dense research digests should use multiple visual aids when helpful. Pipeline 2 is appropriate for article summaries, research maps, operating-point diagrams, comparison maps, and practical checklists.
